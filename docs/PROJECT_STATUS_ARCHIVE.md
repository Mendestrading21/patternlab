# Archive — historique du projet (lot par lot)

> **Ceci est l'archive historique.** L'état courant du dépôt vit dans `docs/PROJECT_STATUS.md`
> (compact, compteurs générés depuis `src/data/repoTruth.ts`). Ce fichier conserve le journal
> chronologique des programmes terminés (v1 Lots 0→19, V5 Lots 0→16, « un signal visuel partout »,
> Experience Max Lots 1→10). Certaines sections ci-dessous décrivent des états **dépassés** (p. ex.
> « un seul monde », « ~8 leçons / ~20 exercices », « 9 formats sur 12 ») : elles sont conservées
> telles quelles comme trace historique et **ne reflètent pas** l'état actuel — se référer à
> `PROJECT_STATUS.md` et `REPO_TRUTH.md` pour les chiffres à jour.
